...
- The app is now built from an Xcode project (generated from `project.yml` with XcodeGen); the release script archives, signs, notarizes and staples through Xcode. A `.mcp.json` connects MCP-capable coding agents to Xcode's MCP tools for contributors.
